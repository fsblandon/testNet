...
export function partOne(companieS) {
  companieS.filter( (c) => {
    c.users.filter( (u) => {
      u.firstName == undefined ? u.firstName = '' : u.firstName = u.firstName;
      u.lastName == undefined ? u.lastName = '' : u.lastName = u.lastName;
    });
  });

  companieS.filter( (c) => {
    const change = c.name[0].toUpperCase();
    c.name = change.concat(c.name.slice(1, c.name.length));
  });

  companieS.filter( (c) => {
    c.users.filter( (u) => {
      if (u.firstName != '') {
        const nameChanged = u.firstName[0].toUpperCase();
        u.firstName = nameChanged.concat(u.firstName.slice(1, u.firstName.length));
      }

      if (u.lastName != '') {
        const lastNameChanged = u.lastName[0].toUpperCase();
        u.lastName = lastNameChanged.concat(u.lastName.slice(1, u.lastName.length));
      }
    });
  });

  companieS.sort( (a, b) => {
    b.usersLength > a.usersLength ? true : false;
  });
  return companieS;
}

console.log('---- EXAMPLE 1 --- ', partOne(companies));
